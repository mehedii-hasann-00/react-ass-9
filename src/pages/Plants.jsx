...
function Plants() {
  ...
  const handle_search = (val) => {
    setSearchText(() => {
      setSearchedApp(apps.filter(app => (app.title).toLowerCase().includes(val.toLowerCase())));
      return val;
    });
  }
  return (
      ...
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="mt-8 flex flex-col-reverse gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p class="text-slate-600 font-medium">
            <span class="font-semibold">({searchedApp.length > 0 ? searchedApp.length : searchText.length === 0 ? apps.length : 0})</span> Apps Found
          </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-10 px-8">
        {searchedApp && searchedApp.length > 0 ?
          searchedApp.map((app, index) => (
            <Link to={`/apps/${app.id}`} key={app.id} className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
          ...
          searchText.length === 0 ?
            ...
            :
            <div className='text-red-500 text-lg text-center'>No App Found...</div>
        }
